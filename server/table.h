...
int set_msg_from_room_t(struct msg_from_room_t* msg, int num, struct sendinf* inf){
	if(msg != NULL){	
		msg->num = num;
		msg->comand = 0;
		if(inf != NULL){
...
static int get_best_comb(int** arr, int num, int comb, int res[6], int md){
	...
	for(int i = 0; i < 5; i++){
		int mx = 0;
		for(int j = 0; j < num; j++){
			if(((temp[j][i] > mx && mx != CARD_A) || temp[j][i] == CARD_A) && res[j] == 1){
				mx = temp[j][i];
			}
		}
		log(md, "%d ", mx);
...
int pokroom::put_money(){
	...
	for(int i = 1; i <= 6; i++){
		...
		for(int j = 0; j < 6; j++){
			if(this->final_table[j] == i){
				this->pupi[j].cash += (this->all_in_table[j] != -1)?(this->all_in_table[j]/count_of_winer):(forone);
				this->puti.bank -= (this->all_in_table[j] != -1)?(this->all_in_table[j]/count_of_winer):(forone);
			}
		}
		for(int j = 0; j < 6; j++){
...
int pokroom::get_winer(){
	...
	for(int i = 0; i < 6; i++){
		if(this->pupi[i].status != STATUS_IN_GAME && this->pupi[i].status != STATUS_ACTIVE){
	...
	while(count_of_player != 0){
		for(int i = 1; i <= 10; i++){
			...
			int num = -1;
			for(int j = 0; j < 6; j++){
				if(this->pupi[j].comb == i && this->pupi[j].status == STATUS_AFTER_GAME && this->final_table[j] == 0){
					count++;
					num = j;
				}
			}
...
int pokroom::start_game(){
	...
	this->puti.bank = this->pupi[this->lit_blind].bet + this->pupi[this->big_blind].bet;
...
int pokroom::all_in(int num){
	...
	this->puti.bet = (this->puti.bet > this->pupi[num].bet)?(this->puti.bet):(this->pupi[num].bet);
...
int pokroom::fold(int num){
	...
	if(this->active_player != num && this->pupi[num].status != STATUS_NON_PLAYER){
		prsend(num);
		return 0;
	}
...
int pokroom::rais(int num, double plus){
	...
	log(this->client->fd_poker_log, "rais %d %lf %lf %lf %lf\n", num, plus, this->pupi[num].cash, this->puti.bet, this->pupi[num].bet);
	if((this->pupi[num].cash <= (this->puti.bet + plus - this->pupi[num].bet)) || (this->puti.bet + plus < this->start_bet)){
		return this->all_in(num);
	}
	this->puti.bet += plus;
...
int pokroom::chplayer(int call_cheak){
	...
	int j = 0;
	while((this->pupi[(this->active_player + j) % 6].status != STATUS_IN_GAME) && (j < 6)) 
		j++;
	...
	log(this->client->fd_poker_log, "%d %lf\n", this->active_player, this->all_in_table[this->active_player]);

	if((this->action_play[this->active_player]) && ((this->pupi[this->active_player].bet == this->puti.bet)) && (call_cheak)){
...
int pokroom::count_all_in(){
	for(int i = 0; i < 6; i++){
		if(this->all_in_table[i] == -2){
			...
			for(int j = 0; j < 6; j++){
				this->all_in_table[i] += ((this->pupi[i].bet - this->pupi[j].bet) < 0)?(this->pupi[i].bet - this->pupi[j].bet):0;
			}
		}
	}
	return 0;
...
int pokroom::game(){
	...
	switch(this->status){
		...
		case ROOM_FIRST_RAUND:
			while(this->pupi[this->active_player].status != STATUS_ACTIVE && this->status == ROOM_FIRST_RAUND){
				this->chplayer(1);
				this->sendtoall();
			}
			break;
		...
		case ROOM_END_GAME:
			...
			for(int i = 0; i < 6; i++){
				log(this->client->fd_poker_log, "#RES %d %s %d %d %lf\n", i, this->pupi[i].login, this->final_table[i], this->pupi[i].comb, this->pupi[i].cash);
			}
...
//copy all players
	for(int j = 0; j < 6; j++){
		...
		strncpy(dest->player[j].login, this->pupi[j].login, sizeof(this->pupi[j].login));
	}
	msg_send.comand = SEND_INFO_TO_CLIENT;
	msg_send.num = this->client->num[i];
	msgsnd(this->message_discrip, (void*)(&msg_send), sizeof(msg_send) - sizeof(long), 0);
	return 0;
}
