...
static int copy_publplayinfo(struct publplayinfo * dest, struct publplayinfo * source){
...
static int copy_publtableinfo(struct publtableinfo * dest, struct publtableinfo * source){
	if(dest == NULL || source == NULL)
		return 1;

	for(size_t i = 0; i < 5; i++)
		dest->card[i] = source->card[i];
...
//copy all players
	for(int i = 0 ; i < 6; i++){
		...
		strncpy(dest->player[i].login, source->player[i].login, sizeof(source->player[i].login));
	}
	return 0;
}
...
static int set_recivesock(struct recivesock* rec, long code, struct sendinf* inf){
	rec->code = code;
	rec->id = 0;
	if(inf != NULL){
