...
int client::tester(int* allcards){
	...
	if (this->circ[0] != 0){
		...
		if (win < (1 - wins)*bet_cur)
			act = 0;
		if (((1 - wins)*(bet_cur + this->table.blind) > win) && (win >= (1-wins)*bet_cur))
			act = 1;
		if (win >= (1-wins)*(bet_cur + this->table.blind))
			act = 2;
	}
	else {
	...
	if (act == 2){
		...
		int limp = 0;
		for (int i = 0; i < 6; i++)
			if (this->player[i].last_action == ACTION_RAISE || this->player[i].last_action == ACTION_CALL)
				limp++;
		//printf("limp %d\n", limp);
		log(this->fd, "old_bet - %lf, table_blind %lf\n", this->old_bet, this->table.blind);
		...
		else if (this->player[this->num].cash < (this->table.bet + 4*this->table.blind - this->player[this->num].bet) || (this->table.bet + 4*this->table.blind - this->player[this->num].bet < this->table.blind))
			return ACTION_CALL;
		else 
			return ACTION_FOLD;
