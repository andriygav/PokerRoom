...
int is_Kicker(struct card a[]) { return a[0].value; }

int is_OnePair(struct card a[]) {
  for (int i = 0; i < 6; i++) {
    if (a[i].value == a[i + 1].value && (a[i].num != -1)) return a[i].value;
  }
  return -1;
}

int is_TwoPairs(struct card a[]) {
  int fir_pair = 0;
  for (int i = 0; i < 6; i++) {
...
int is_Set(struct card a[]) {
  for (int i = 0; i < 5; i++) {
    if ((a[i].value == a[i + 1].value) && (a[i + 2].value == a[i + 1].value) &&
        (a[i].num != -1))
      return a[i].value;
  }
  return -1;
...
int is_Straight(struct card a[]) {
  int count = 0;
  for (int i = 0; i < 6; i++) {
    if ((a[i].value - a[i + 1].value <= 1)) {
      count++;
      if (count == 5) return a[i - 4].value;
    } else
      count = 0;
  }
  return -1;
...
int is_Flush(struct card a[]) {
  ...
  for (int i = 0; i < 4; i++)
    if (count[i] >= 5)
      for (int j = 0; j < 3; j++)
        if (a[j].suit == i) return a[i].value;

  return -1;
}

int is_FullHouse(struct card a[]) {
  int set = is_Set(a);
  int pair = is_OnePair(a);
  if (set && pair && set != pair) return set;
  return -1;
}

int is_Quads(struct card a[]) {
  for (int i = 0; i < 3; i++) {
    if ((a[i].value == a[i + 1].value) && (a[i + 2].value == a[i + 1].value) &&
        (a[i + 2].value == a[i + 3].value) && (a[i].num != -1))
      return a[i].value;
  }
  return -1;
}

int is_StraightFlush(struct card a[]) {
  int count = 0;
  for (int i = 0; i < 6; i++) {
...
int is_RoyalFlush(struct card a[]) {
  if (is_Straight(a) == CARD_A && is_Flush(a) == CARD_A) return CARD_A;
  return -1;
}

void sortcard(struct card a[]) {
...
int count_comb(struct card arr[]) {
  int ot = 0;
  ot = is_RoyalFlush(arr);
  if (ot != -1) {
    return COMB_FLASH_ROYAL;
  }
  ot = is_StraightFlush(arr);
  if (ot != -1) {
