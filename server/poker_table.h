...
void* get_info_from_client(void* argv) {
  ...
  while (1) {
    if (msgrcv(cl->PokMesDis, (void*)(&msg_recive), sizeof(msg_recive.buf),
               i + MAX_CLIENT_NUM, 0) == -1) {
      log(cl->fd_poker_log, "msgrcv on poker: %s\n", strerror(errno));
      return nullptr;
    }
    log(cl->fd_poker_log, "recive info from %ld %ld %lf\n", cl->num[i],
        msg_recive.buf.comand, msg_recive.buf.rs);
...
void* poker(void* argumets) {
  ...
  for (int i = 0; i < 6; i++) {
    argv = (struct get_info_from_client_argumets_t*)malloc(
        sizeof(struct get_info_from_client_argumets_t));
    ((struct get_info_from_client_argumets_t*)argv)->i = i;
    ((struct get_info_from_client_argumets_t*)argv)->mut =
  ...
  argv = (struct get_info_from_server_argumets_t*)malloc(
      sizeof(struct get_info_from_server_argumets_t));
